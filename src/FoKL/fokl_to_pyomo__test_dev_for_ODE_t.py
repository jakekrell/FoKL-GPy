...
def _add_gp(self, xvars, yvar, m, xfix, yfix, truescale, std, draws, igp, t_span):
    """
    ...
    if t_span is not None:
        m.add_component(f"GP{igp}_t", dae.ContinuousSet(bounds=(t_span[0], t_span[1])))  # continuous index for ODE time
    else:
        m.add_component(f"GP{igp}_t", pyo.Set(initialize=0))  # stand-in empty index for where ODE time dimension would be

    # Define some Pyomo vars:
    ...
    if m.find_component(yvar) is None:  # then define; else a previous model already defined this variable
        m.add_component(yvar, pyo.Var(m.component(f"GP{igp}_t"), within=pyo.Reals))  # FoKL output, as single variable equal across GP's

    m.add_component(f"GP{igp}_{yvar}_mean", pyo.Var(m.component(f"GP{igp}_t"), within=pyo.Reals))  # FoKL output, mean of draws for current GP
    if std is True:
        m.add_component(f"GP{igp}_{yvar}_draw", pyo.Var(m.component(f"GP{igp}_t"), m.component(f"GP{igp}_scenarios"), within=pyo.Reals))  # FoKL output, evaluated at draw for current GP
        m.add_component(f"GP{igp}_{yvar}_std", pyo.Var(m.component(f"GP{igp}_t"), within=pyo.Reals))  # FoKL output, standard deviation of draws for current GP
        m.add_component(f"GP{igp}_{yvar}_var", pyo.Var(m.component(f"GP{igp}_t"), within=pyo.Reals))  # FoKL output, variance of draws for current GP

    # Define FoKL normalized inputs (and their true scale counterparts) as Pyomo variables:

    for j in m.component(f"GP{igp}_j"):
        m.add_component(f"GP{igp}_{xvars[j]}_norm", pyo.Var(m.component(f"GP{igp}_t"), within=pyo.Reals, bounds=[0, 1], initialize=0.5))  # FoKL input variables
        ...
        if truescale[j] is True:  # create expression relating normalized variable to true scale
            if m.find_component(xvars[j]) is None:  # confirm truescale variable was not previously defined in prior GP
                m.add_component(xvars[j], pyo.Var(m.component(f"GP{igp}_t"), within=pyo.Reals, bounds=self.minmax[j], initialize=sum(self.minmax[j]) / 2))
            ...
            if m.find_component(xvars[j]) is None:
                m.add_component(xvars[j], pyo.Var(m.component(f"GP{igp}_t")))  # to be set equal to '{xvar}_norm' in later constraint (where normalization constraint is applied)
    ...
    # =====================================================================================================
    # =====================================================================================================
    # RTW ... copy/paste "m.component(f"GP{igp}_t"), " into variable definitions
...
def fokl_to_pyomo(models, xvars, yvars, m=None, xfix=None, yfix=None, truescale=True, std=True, draws=None, t_span=None):
    """
    ...
    for im in range(len(models)):  # index of model
        ...
        # Add GP to Pyomo:
        m = _add_gp(models[im], xvars[im], yvars[im], m, xfix[im], yfix[im], truescale[im], std[im], draws[im], igp, t_span)
