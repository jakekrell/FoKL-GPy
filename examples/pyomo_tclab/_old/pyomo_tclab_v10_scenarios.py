...
for draws in draws_list:
    ...
    m.s = pyo.Set(initialize=range(5))  # scenarios

    m.TmTamb = pyo.Var(m.t, m.s, bounds=GP.minmax[0])  # == T - Tamb
    
    # m.u = pyo.Var(m.t, bounds=(0, 100))  # same across scenarios; not yet supported for xvars to be diff. m.t, m.s, etc.
    m.u = pyo.Var(m.t, m.s, bounds=(0, 100))  # same across scenarios; need constr.
    
    m.dT = dae.DerivativeVar(m.TmTamb, wrt=m.t)
    
    for s in m.s:
        m.TmTamb[r_t0, s].fix(0.0)
    
    GP.to_pyomo([m.TmTamb, m.u], m.dT, m, draws, with_blocks=False, scenarios=m.s)

    # ENFORCE m.u IS SAME ACROSS m.s

    # LIKE SO (can be written in Pyomo for all s but skipping first one; then constr. of s_first = actual u data value):
    u[t, 0] == u[t, 1]
    u[t, 0] == u[t, 2]
    u[t, 0] == u[t, 3]
